...
export class GitHubProvider extends Utils {
    ...
    */
    ...
    public async createPromotionPullRequest(githubOrganization: string, repositoryName: string, sourceEnvironment: string, targetEnvironment: string): Promise<[string, number]> {
        const getImage = await this.extractImageFromContent(githubOrganization, repositoryName, repositoryName, sourceEnvironment);

        if (getImage === undefined) {
            throw new Error("Failed to create a pr");
        }

        const gitopsPromotionPR = await this.promoteGitopsImageEnvironment(githubOrganization, repositoryName, repositoryName, targetEnvironment, getImage);
