...
import { createTaskCreatorOptionsGitlab } from '../../../src/utils/test.utils';
import { Kubernetes } from '../../../src/apis/kubernetes/kube';
import { generateRandomChars } from '../../../src/utils/generator';


export class GitlabController extends Utils implements GitController {
    ...
    public async createTaskCreatorOptions(softwareTemplateName: string, imageName: string, imageOrg: string, imageRegistry: string, gitOrganization: string, repositoryName: string, componentRootNamespace: string, ciType: string): Promise<ScaffolderScaffoldOptions> {
        return createTaskCreatorOptionsGitlab(softwareTemplateName, imageName, imageOrg, imageRegistry, gitOrganization, repositoryName, componentRootNamespace, ciType);
    }
    
    //TODO: Implement
    public async updateRoxCentralEndpoint(_gitOrganization: string, _repositoryName: string, _roxCentralEndpoint: string): Promise<string | undefined> {
        return undefined;
    }

    //TODO: Implement
    public async deleteCosignPublicKey(_gitOrganization: string, _gitRepository: string): Promise<string | undefined> {
        return undefined;
    }

    //TODO: Implement
    public async checkEnvVariables(_componentRootNamespace: string, _gitOrganization: string, _imageOrg: string, _ciNamespace: string, _kubeClient: Kubernetes): Promise<void> {
        return;
    }
    ...
    public async checkIfRepositoryExists(organization: string, repoName: string): Promise<boolean> {
    ...
    public async getProjectId(organization: string, repoName: string): Promise<string> {
        //RHTAPBUGS-1327: Added wait: it should improve stability of Gitlab test - sometimes request from tests could be faster, than GitLab responses
        while (true) {
            try {
                const projects = await this.gitlab.Projects.show(`${organization}/${repoName}`);
                ...
                throw new Error(`Failed to get id for ${organization}/${repoName}`);
            }
        }
    }
    /**
     ...
     */
    public async checkIfFolderExistsInRepository(gitOrganization: string, repositoryName: string, folderPath: string): Promise<boolean> {
    ...
    public async extractImageFromContent(gitOrganization: string, repositoryName: string, componentName: string, environment: string) {
        ...
        try {
            const environmentContent = await this.gitlab.RepositoryFiles.showRaw(repositoryID,
                `components/${componentName}/overlays/${environment}/deployment-patch.yaml`, 'main');
    ...
    public async promoteGitopsImageEnvironment(gitOrganization: string, repositoryName: string,
        componentName: string, environment: string, image: string): Promise<number> {
        const repositoryID = await this.getProjectId(gitOrganization, repositoryName);

        const targetBranch = generateRandomChars(6);
        ...
        try {
            ...
            const targetEnvironmentContent = await this.gitlab.RepositoryFiles.showRaw(repositoryID,
                `components/${componentName}/overlays/${environment}/deployment-patch.yaml`, targetBranch);

            const targetEnvironmentContentToString = targetEnvironmentContent.toString();

            const pattern = /- image: (.*)/;
            const newContent = targetEnvironmentContentToString.replace(pattern, `- image: ${image}`);
            ...
            await this.gitlab.Commits.create(
                ...
                [
                    {
                        action: 'update',
                        filePath: `components/${componentName}/overlays/${environment}/deployment-patch.yaml`,
                        content: newContent
                    },
                ]
            );
    ...
    public async createWebhook(owner: string, repo: string, webHookUrl: string): Promise<ProjectHookSchema> {
        const repositoryID = await this.getProjectId(owner, repo);
    ...
    public async createPullRequestFromMainBranch(organization: string, repositoryName: string, _title: string, _content: string, _fileSHA: string): Promise<number> {
        const title = 'Automatic Merge Request created from testing framework';
        const branchName = generateRandomChars(6);
        const repositoryID = await this.getProjectId(organization, repositoryName);
        try {
            const mainBranch = await this.gitlab.Branches.show(repositoryID, 'main');
    ...
    public async mergePullRequest(gitOrganization: string, repositoryName: string, mergeRequestId: number) {
    ...
    public async updateEnvFileForGitLabCI(gitOrganization: string, repositoryName: string, rekorHost: string, tufMirror: string): Promise<boolean> {
        ...
        const fileContent = await this.getFileContentAsString(repositoryID, 'main', filePath);
        // Replace ACS
        ...
        //Commit changed file
        return await this.commitFileContent(repositoryID, 'main', filePath, "Update env file for GitLabCI", updatedContent);
    }


    public async commitReplacementStringInFile(gitOrganization: string, repositoryName: string, filePath: string, commitMessage: string, textToReplace: string, replacement: string): Promise<string> {
        const repositoryID = Number(await this.getProjectId(gitOrganization, repositoryName));
    ...
    public async getFileContentAsString(gitOrganization: string, repositoryName: string, filePath: string): Promise<string> {
        const repositoryID = await this.getProjectId(gitOrganization, repositoryName);

        try {
            // Get the current content of the file
            const file = await this.gitlab.RepositoryFiles.show(repositoryID, filePath, 'main');
    ...
    public async commitFileContent(gitOrganization: string, repositoryName: string, filePath: string, commitMessage: string, fileContent: string): Promise<boolean> {
        const repositoryID = await this.getProjectId(gitOrganization, repositoryName);
