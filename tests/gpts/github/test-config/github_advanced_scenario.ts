...
export const githubSoftwareTemplatesAdvancedScenarios = (gptTemplate: string) => {
    describe(`Red Hat Trusted Application Pipeline ${gptTemplate} GPT tests GitHub provider with public/private image registry`, () => {
        ...
        let kubeTestBlocks: TektonTestBlocks;
        ...
        */
        beforeAll(async () => {
            ...
            kubeTestBlocks = new TektonTestBlocks(kubeClient);
        ...
        it(`Creates a pull request and verify a PipelineRun started`, async () => {
            ...
            expect(await kubeTestBlocks.verifyPipelineRunStart(repositoryName, 'pull_request')).toBe(true);
        }, 120000);
        ...
        it(`Wait component ${gptTemplate} pipelinerun to be triggered and finished`, async () => {
            expect(await kubeTestBlocks.verifyPipelineRunSuccess(repositoryName, ciNamespace, 'pull_request')).toBe(true);
        }, 900000);
        ...
        it(`Merge pull_request to trigger a push pipelinerun`, async () => {
            ...
            expect(await kubeTestBlocks.verifyPipelineRunStart(repositoryName, 'push')).toBe(true);
        }, 120000);
        ...
        it(`Wait component ${gptTemplate} push pipelinerun to be triggered and finished`, async () => {
            expect(await kubeTestBlocks.verifyPipelineRunSuccess(repositoryName, ciNamespace, 'push')).toBe(true);
        }, 900000);
        ...
        it(`Check ${gptTemplate} pipelinerun yaml has the rh-syft image path`, async () => {
            expect(await kubeTestBlocks.verifySyftImagePath(repositoryName, ciNamespace, 'push')).toBe(true);
        }, 900000);
        ...
        it(`Check if ACS Scan is successful for ${gptTemplate}`, async () => {
            expect(await kubeTestBlocks.checkIfAcsScanIsPass(repositoryName, ciNamespace, 'push')).toBe(true);
        }, 900000);
        ...
        it('trigger pull request promotion to promote from development to stage environment', async () => {
            [extractedBuildImage, gitopsPromotionPRNumber] = await gitHubClient.createPromotionPullRequest(githubOrganization, repositoryName, developmentEnvironmentName, stagingEnvironmentName);
            expect(await kubeTestBlocks.verifyPipelineRunStart(repositoryNameGitops, 'pull_request'));
        });
        ...
        it('verifies successful completion of EC PipelineRun to ensure environment promotion from development to staging', async () => {
            expect(await kubeTestBlocks.verifyPipelineRunSuccess(repositoryNameGitops, ciNamespace, 'pull_request'));
        }, 900000);
        ...
        */
        it('trigger pull request promotion to promote from stage to prod environment', async () => {
            [extractedBuildImage, gitopsPromotionPRNumber] = await gitHubClient.createPromotionPullRequest(githubOrganization, repositoryName, stagingEnvironmentName, developmentEnvironmentName);
            expect(await kubeTestBlocks.verifyPipelineRunStart(repositoryNameGitops, 'pull_request'));
        });
