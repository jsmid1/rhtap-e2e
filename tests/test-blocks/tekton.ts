...
import { TektonUtils } from '../../src/utils/tekton-utils';


export class TektonTestBlocks {

    private readonly kubeClient;
    private readonly tektonUtils;
    ...
    constructor(kubeClient: Kubernetes) {
        ...
        this.tektonUtils = new TektonUtils();
    }
     ...
     * 
     * @param {string} repositoryName - name of the repository for which the pipeline run is triggered
     * @param {string} eventType - type of the event which triggered the pipeline
     * @returns 
     */
    ...
    /**
     * Waits until a pipeline run is created in the cluster and start to wait until succeed/fail,
     * checks expected TaskRuns were completed and collects logs.
     * 
     * @param {string} repositoryName - name of the repository for which the pipeline run is triggered
     * @param {string} namespace - namespace of the pipelineRun
     * @param {string} eventType - type of the event which triggered the pipeline
     * @param {string[]} expectedTasks - tasks that are expected to be run during the pipelineRun
     * @returns {Promise<boolean>} - true if pipelineRun and all expected taskRuns were succesful, false otherwise
     */
    public async verifyPipelineRunSuccess(repositoryName: string, namespace: string, eventType: string, expectedTasks: string[]): Promise<boolean> {
        ...
        if (pipelineRun?.metadata?.name) {
            ...
            await this.tektonUtils.logTaskRuns(pipelineRun, namespace);
            return finished && await this.tektonUtils.checkTaskRuns(pipelineRun, expectedTasks);
        }
